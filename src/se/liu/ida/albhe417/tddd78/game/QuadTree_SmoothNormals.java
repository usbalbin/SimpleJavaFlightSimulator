 ...
 */
public class QuadTree_SmoothNormals {
    //TODO implement me
    //Info: http://victorbush.com/2015/01/tessellated-terrain/

    private QuadTree_SmoothNormals leftFront;
    private QuadTree_SmoothNormals rightFront;
    private QuadTree_SmoothNormals leftBottom;
    private QuadTree_SmoothNormals rightBottom;
    private QuadTree_SmoothNormals parent;
    ...
    */
    ...
    private static QuadTree_SmoothNormals root;

    /**
     * Create Root QuadTree
     * @param heightmap heightmap
     */
    public QuadTree_SmoothNormals(float[] heightmap, final float heightFactor, float maxHeight){
     ...
     */
    private QuadTree_SmoothNormals(final Vector3 position, final int size, final short level, final Vector3 cameraPos, final float detailFactor, final short maxLevels, QuadTree_SmoothNormals parent) {
    ...
    private void generateTree(final Vector3 cameraPos, final float detailFactor, final short maxLevels){
        ...
        if(leftFront == null && rightFront == null && leftBottom == null && rightBottom == null) {
            leftFront = new QuadTree_SmoothNormals(position.add(new Vector3(-halfChildSize, 0, -halfChildSize)), childSize, childLevel, cameraPos, detailFactor, maxLevels, this);
            rightFront = new QuadTree_SmoothNormals(position.add(new Vector3(+halfChildSize, 0, -halfChildSize)), childSize, childLevel, cameraPos, detailFactor, maxLevels, this);
            leftBottom = new QuadTree_SmoothNormals(position.add(new Vector3(-halfChildSize, 0, +halfChildSize)), childSize, childLevel, cameraPos, detailFactor, maxLevels, this);
            rightBottom = new QuadTree_SmoothNormals(position.add(new Vector3(+halfChildSize, 0, +halfChildSize)), childSize, childLevel, cameraPos, detailFactor, maxLevels, this);
        }
        else {
     ...
     */
    //TODO fix me
    protected void stitch(QuadTree_SmoothNormals neighborLeft, QuadTree_SmoothNormals neighborFront, QuadTree_SmoothNormals neighborRight, QuadTree_SmoothNormals neighborBottom){
        //Stuff....
        //..
        //..
        if(hasChildren()){
    ...
    private void stitchLeft(final QuadTree_SmoothNormals leftNeighbor){
    ...
    private void stitchFront(final QuadTree_SmoothNormals frontNeighbor){
        //If our neighbor has children then either the matching quad has same(do nothing) or higher
        // LOD-level(neighbor will do the stitching)
    ...
    private void stitchRight(final QuadTree_SmoothNormals rightNeighbor){
        //If our neighbor has children then either the matching quad has same(do nothing) or higher
        // LOD-level(neighbor will do the stitching)
    ...
    private void stitchBottom(final QuadTree_SmoothNormals bottomNeighbor){
        //If our neighbor has children then either the matching quad has same(do nothing) or higher
        // LOD-level(neighbor will do the stitching)
    ...
    protected void generateVerticesAndIndices(final List<VertexPositionColorNormal> vertices, final List<Integer> indices){
        if(hasChildren()){
            ...
            if(!isStitched()){
                ...
                int leftFrontIndex = vertices.lastIndexOf(new VertexPositionColorNormal(leftFrontPos, null));
                if(leftFrontIndex == -1) {
                    vertices.add(new VertexPositionColorNormal(leftFrontPos, color));//0
                    leftFrontIndex = vertices.size() - 1;
                }

                int rightFrontIndex = vertices.lastIndexOf(new VertexPositionColorNormal(rightFrontPos, null));
                if(rightFrontIndex == -1) {
                    vertices.add(new VertexPositionColorNormal(rightFrontPos, color));//1
                    rightFrontIndex = vertices.size() - 1;
                }

                int leftBottomIndex = vertices.lastIndexOf(new VertexPositionColorNormal(leftBottomPos, null));
                if(leftBottomIndex == -1) {
                    vertices.add(new VertexPositionColorNormal(leftBottomPos, color));//2
                    leftBottomIndex = vertices.size() - 1;
                }

                int rightBottomIndex = vertices.lastIndexOf(new VertexPositionColorNormal(rightBottomPos, null));
                ...
                int i = 0;
                int[] quadsIndices = new int[numVertices];

                vertices.add(new VertexPositionColorNormal(center, color));
                quadsIndices[i] = vertices.size() - 1;

                quadsIndices[++i] = vertices.lastIndexOf(new VertexPositionColorNormal(leftFrontPos, null));
                if(quadsIndices[i] == -1) {
                ...
                if(frontStitchPnt) {
                    quadsIndices[++i] = vertices.lastIndexOf(new VertexPositionColorNormal(frontPos, null));
                    if(quadsIndices[i] == -1) {
                ...
                quadsIndices[++i] = vertices.lastIndexOf(new VertexPositionColorNormal(rightFrontPos, null, null));
                if(quadsIndices[i] == -1) {
                    vertices.add(new VertexPositionColorNormal(rightFrontPos, color));
                    quadsIndices[i] = vertices.size() - 1;
                }
                if(rightStitchPnt) {
                    quadsIndices[++i] = vertices.lastIndexOf(new VertexPositionColorNormal(rightPos, null));
                    if(quadsIndices[i] == -1) {
                ...
                quadsIndices[++i] = vertices.lastIndexOf(new VertexPositionColorNormal(rightBottomPos, null));
                if(quadsIndices[i] == -1) {
                    vertices.add(new VertexPositionColorNormal(rightBottomPos, color));
                    quadsIndices[i] = vertices.size() - 1;
                }
                if(bottomStitchPnt) {
                    quadsIndices[++i] = vertices.lastIndexOf(new VertexPositionColorNormal(bottomPos, null));
                    if(quadsIndices[i] == -1) {
                        vertices.add(new VertexPositionColorNormal(bottomPos, color));
                        quadsIndices[i] = vertices.size() - 1;
                    }
                }
                quadsIndices[++i] = vertices.lastIndexOf(new VertexPositionColorNormal(leftBottomPos, null, null));
                if(quadsIndices[i] == -1) {
                    vertices.add(new VertexPositionColorNormal(leftBottomPos, color));
                    quadsIndices[i] = vertices.size() - 1;
                }
                if(leftStitchPnt) {
                    quadsIndices[++i] = vertices.lastIndexOf(new VertexPositionColorNormal(leftPos, null));
                    if(quadsIndices[i] == -1) {
...
/*private QuadTree getNortNeighbor(QuadType currentQuadType, QuadTree parent){
    ...
    protected QuadTree_SmoothNormals findNode(final Vector3 position){
